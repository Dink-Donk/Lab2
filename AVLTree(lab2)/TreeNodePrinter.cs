...
namespace AVLTree
{
    ...
    public static class TreeNodePrinter
    {
        public static void Print<TKey, TValue>(this Node<TKey, TValue> root, int topMargin = 2, int leftMargin = 2)
        {
            ...
            {
                var item = new NodeInfo<TKey, TValue> { Node = next, Text = string.Format("({0}:h{1})", next.Key.ToString()) };
                if (level < last.Count)
                {
        ...
        private static void Print<TKey, TValue>(NodeInfo<TKey, TValue> item, int top)
        {
            SwapColors();
